	...
	return (
		...
		<div className='text-gray-100 flex items-center max-w-[1240px] w-screen mx-auto h-[70px] justify-between text-[15px] font-medium'>
			...
			<div className="py-3 rounded-md ">
				<button onClick={handleSignClick('login')} className="bg-slate-600 p-2 px-3 rounded-2xl hover:bg-slate-700 transition duration-200 ease-in-out">Вход</button>
				<button onClick={handleSignClick('register')} className="p-2 rounded-md hover:text-gray-300 transition ease-in-out duration-200">Регистрация</button>
			</div>
			</div>
		</div>
		</div>
