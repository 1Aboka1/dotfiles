...
var getWebviewAPI = () => {
  ...
  if (!webviewExt) {
    ...
    void import_coc15.window.showErrorMessage(prompt);
...
async function pasteImageFile(sourceUri, imageFilePath) {
  ...
  fs.mkdir(assetDirectoryPath, () => {
    fs.stat(destPath, (err) => {
      ...
      void import_coc16.window.showInformationMessage(
      ...
      doc.applyEdits([import_coc16.TextEdit.insert(pos, `![${description}](${url})`)]).catch(logger.error);
    });
  });
}
  ...
  if (textLine.indexOf(hint) >= 0) {
    doc.applyEdits([
      import_coc16.TextEdit.replace(
        import_coc16.Range.create(
          import_coc16.Position.create(line, 0),
          import_coc16.Position.create(line, textLine.length)
        ),
...
var MarkdownPreviewEnhancedView = class {
  constructor(context) {
    ...
    mume.init(this.config.configPath).then(async () => {
      ...
      (0, import_utility.useExternalAddFileProtocolFunction)(
        (filePath, preview) => {
          if (preview) {
...
`) {
        ...
        await doc2.applyEdits([
          import_coc18.TextEdit.replace(
            import_coc18.Range.create(
              import_coc18.Position.create(start + 2, 0),
              import_coc18.Position.create(end - 1, 0)
            ),
            `${result}
`
          )
        ...
        await doc2.applyEdits([
          import_coc18.TextEdit.insert(
            import_coc18.Position.create(i + 1, 0),
            `
<!-- code_chunk_output -->
...
`
    ...
    if (this.formatPathIfNecessary(import_coc18.Uri.parse(doc.uri).fsPath) === filePath) {
    ...
    if (!engine) {
      engine = new import_mume2.MarkdownEngine({
        ...
        projectDirectoryPath: this.getProjectDirectoryPath(
          sourceUri,
          import_coc18.workspace.workspaceFolders
        ),
        config: this.config
      });
  ...
  async initPreview(sourceUri, doc, openURL) {
    ...
    if (isUsingSinglePreview && this.singlePreviewPanel) {
      ...
      const localResourceRoots = [
        ...
        // Uri.file(tmpdir()),
        import_coc18.Uri.file(
          this.getProjectDirectoryPath(sourceUri, import_coc18.workspace.workspaceFolders) || import_path.default.dirname(sourceUri.fsPath)
        )
      ];
      ...
      previewPanel.iconPath = import_coc18.Uri.file(
  ...
  htmlExport(sourceUri, offline) {
    ...
    if (engine) {
      engine.htmlExport({ offline }).then((dest) => {
        void import_coc18.window.showInformationMessage(
...
`
  ...
  async function openImageHelper() {
    await contentProvider.openImageHelper(
      import_coc19.Uri.parse((await import_coc19.workspace.document).uri)
    );
  }
  ...
  async function clickTagA(sourceUri, href) {
    ...
    if (resourceUri == null ? void 0 : resourceUri.localPath) {
      ...
      const hrefUri = import_coc19.Uri.parse(resourceUri.localPath);
    ...
    await doc.applyEdits([
      import_coc19.TextEdit.replace(
        import_coc19.Range.create(
          import_coc19.Position.create(dataLineNum, 0),
          import_coc19.Position.create(dataLineNum, line.length)
        ),
        line
      )
    ]);
  ...
  context.subscriptions.push(
    import_coc19.workspace.onDidSaveTextDocument(
    ...
    import_coc19.workspace.onDidChangeTextDocument(
    ...
    import_coc19.workspace.onDidChangeConfiguration(
      logger.asyncCatch(() => {
        contentProvider.updateConfiguration();
      })
    ),
    import_coc19.events.on(
      "CursorMoved",
    ...
    import_coc19.events.on(
      ...
      logger.asyncCatch(async () => {
        ...
        if (isMarkdownFile(doc.textDocument)) {
          const sourceUri = import_coc19.Uri.parse(doc.uri);
          const config = import_coc19.workspace.getConfiguration(
            "markdown-preview-enhanced"
          );
